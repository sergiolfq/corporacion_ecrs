...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="brd1">
			  ...
			  <tr>
				<td class="txt1" id="<?=$AGGvars["hash"]?>_<?=$recurso[$AGGvars["pk"]]?>"><?=$recurso[$AGGvars["campo_buscar"]]?><img src="/images/colors/car_<?=$recurso[$AGGvars["pk"]]?>.jpg" /></td>
				<td align="right" class="txt1"><a href="javascript:void(cargar_<?=$AGGvars["hash"]?>('pk=<?=$recurso[$AGGvars["pk"]]?>&<?=$AGGvars["campo_buscar"]?>=<?=$recurso[$AGGvars["campo_buscar"]]?>&mode=1&AGGvars=<?=urlencode(base64_encode(serialize($AGGvars))) ?>','<?=$recurso[$AGGvars["pk"]]?>'))"><img src="/images/admin/add.png" width="16" height="16" border="0" /></a></td>
			  </tr>
			  <?php } ?>
			  <tr>
...
?>
		...
		<li id="<?=$AGGvars["hash"]?><?=$value[$AGGvars["pk"]]?>" class="lista_<?=$AGGvars["hash"]?>">
		  <table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
			  <td width="90%" align="left"><?=$value[$AGGvars["campo_buscar"]]?><img src="/images/colors/car_<?=$value[$AGGvars["pk"]]?>.jpg"></td>
