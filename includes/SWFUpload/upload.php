...
// Settings
	//$save_path = SERVER_ROOT. "/tmp/chroot/";	// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
	$save_path = SERVER_ROOT . "/descargas/";
	@unlink($save_path . $file_name);
	$upload_name = "Filedata";
	$max_file_size_in_bytes = 2147483647;				// 2GB in bytes
	$extension_whitelist = array("jpg","jpge","png");	// Allowed file extensions
...
// Validate the upload
	if (!isset($_FILES[$upload_name])) {
		HandleError("No se subieron archivos en \$_FILES para " . $upload_name);
		exit(0);
	} else if (isset($_FILES[$upload_name]["error"]) && $_FILES[$upload_name]["error"] != 0) {
		HandleError($uploadErrors[$_FILES[$upload_name]["error"]]);
		exit(0);
	} else if (!isset($_FILES[$upload_name]["tmp_name"]) || !@is_uploaded_file($_FILES[$upload_name]["tmp_name"])) {
		HandleError("Fallo is_uploaded_file test.");
		exit(0);
	} else if (!isset($_FILES[$upload_name]['name'])) {
		HandleError("El archivo no tiene nombre.");
		exit(0);
	}
	
	/*else if(!eregi("image",$_FILES[$upload_name]["type"])){
		HandleError("El archivo no es una imagen");
		exit(0);
	}*/
